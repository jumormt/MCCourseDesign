...
# 产生num*6的BPSK信号
def generate_BPSK(num):
    BPSKS_syb = []

    BPSK_syb = []

    p = np.random.randint(2, size=num*6)  # 0-1的num*6个随机数

    BPSKtmp = np.zeros((num*6, 1))


    for i in p:
        BPSKS_syb.append(bpsk[i])
        BPSK_syb.append(bpsk_syb[i])

    BPSKS_syb = np.array(BPSKS_syb)

    return BPSKS_syb, BPSKtmp, BPSK_syb
...
# 接收端进行BPSK判决(软判决)
def get_BPSKJ(BPSK_syb, BPSKR_syb, bpskj, BPSKtmp):
    BPSKJ_syb = BPSK_syb.copy()
    BPSKJ_syb = np.array(BPSKJ_syb)

    for i in range(len(BPSKR_syb)):
        BPSKtmp[i] = np.sum(BPSKR_syb[i])
    for obj in range(len(BPSKJ_syb)):
        olen1 = np.sqrt(np.sum(np.square(BPSKtmp[obj] - bpskj[0])))
        olen2 = np.sqrt(np.sum(np.square(BPSKtmp[obj] - bpskj[1])))
        a = {olen1: 0, olen2: 1}
        olent1 = min([olen1, olen2])
        BPSKJ_syb[obj] = a[olent1]
    BPSKJ_syb = np.array(BPSKJ_syb)
    print(BPSKJ_syb[:100])
    return BPSKJ_syb
...
# 计算误比特率
def get_BPSK_BER(BPSK_syb, BPSKJ_syb):
    count = 0
    for i in range(len(BPSKJ_syb)):
        if (BPSK_syb[i] != BPSKJ_syb[i]):
            count = count + 1
    BER = count / len(BPSKJ_syb)
    return BER
...
def plot_BER():
    ...
    for i in var:
        ...
        txt = str("归一化的信噪比（DB）：") + str(np.log10(100 / i)) + str("DB") + "\n" + str("误比特率：") + "\n" + str("signal Ⅰ：") + str(BER1) + str("   signal Ⅱ(经过信道编码)：") + str(BER2)  + str("   BPSK：") + str(BERBPSK) + "\n" * 2
        file.write(txt)
        print("---------------------------------")
        print("signal Ⅰ：")
